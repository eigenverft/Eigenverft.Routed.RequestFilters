...
{
    /// <summary>
    /// Middleware that filters HTTP requests based on the <c>Accept-Language</c> header using configured whitelist and blacklist rules.
    /// </summary>
    /// <remarks>
    /// The observed value is derived from the first <c>Accept-Language</c> header value (if present). If missing, the observed value is <see cref="string.Empty"/>.
    /// </remarks>
    public class AcceptLanguageFiltering
    {
        private readonly RequestDelegate _next;
        private readonly IDeferredLogger<AcceptLanguageFiltering> _logger;
        private readonly IOptionsMonitor<AcceptLanguageFilteringOptions> _optionsMonitor;
        private readonly IFilteringEventStorage _filteringEventStorage;

        /// <summary>
        /// Initializes a new instance of the <see cref="AcceptLanguageFiltering"/> class.
        /// </summary>
        /// <param name="nextMiddleware">The next middleware in the pipeline.</param>
        /// <param name="logger">The deferred logger instance.</param>
        /// <param name="optionsMonitor">The options monitor for <see cref="AcceptLanguageFilteringOptions"/>.</param>
        /// <param name="filteringEventStorage">The central filtering event storage.</param>
        public AcceptLanguageFiltering(RequestDelegate nextMiddleware, IDeferredLogger<AcceptLanguageFiltering> logger, IOptionsMonitor<AcceptLanguageFilteringOptions> optionsMonitor, IFilteringEventStorage filteringEventStorage)
        {
            ...
            _optionsMonitor.OnChange(_ => _logger.LogDebug("Configuration for {MiddlewareName} updated.", () => nameof(AcceptLanguageFiltering)));
        }

        /// <summary>
        /// Processes the current request by classifying the <c>Accept-Language</c> value and applying the configured policy.
        /// </summary>
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            AcceptLanguageFilteringOptions options = _optionsMonitor.CurrentValue;

            // Mirror the old behavior: use the first header value if multiple are present; missing => "".
            string observed = context.Request.Headers.AcceptLanguage.FirstOrDefault()?.ToString() ?? string.Empty;
            ...
            {
                FilterDecisionLogEntry log = FilterDecisionLogBuilder.Create(nameof(AcceptLanguageFiltering), context.TraceIdentifier, matchKind, isAllowed: true, observed, loggedForEvaluator: false, options.LogLevelWhitelist, options.LogLevelBlacklist, options.LogLevelUnmatched);
            ...
            if (matchKind == FilterMatchKind.Blacklist)
            {
                if (options.RecordBlacklistedRequests)
                {
                    await _filteringEventStorage.StoreAsync(new FilteringEvent
                    {
                        ...
                        EventSource = nameof(AcceptLanguageFiltering),
                        MatchKind = matchKind,
                ...
                FilterDecisionLogEntry log = FilterDecisionLogBuilder.Create(
                    nameof(AcceptLanguageFiltering),
